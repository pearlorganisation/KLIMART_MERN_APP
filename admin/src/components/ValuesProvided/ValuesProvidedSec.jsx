...
import './ValuesProvided.css'


function ValueProvidedPageSec() {
  const {id} = useParams()
  console.log("id###" , id)
  ...
  const handleSection = async()=>{
    setDescData(false)
    setLoading(true)
    const response = await instance.get(`/valuesProvided/${id}` ,  { withCredentials: true })
    setLoading(false)
  ...
  return loading ? (
    ...
    >
      ...
      <strong>Values Provided Section {index}</strong>
      <div class="card">
        <div class="card-body">
        <img
          src={getData?.Image[0]}
      ...
      <CommonModal getData={getData} onUpdateData = {handleSection} pathUrl= "valuesProvided"/>
    </div>
      </>
...
export default ValueProvidedPageSec;
