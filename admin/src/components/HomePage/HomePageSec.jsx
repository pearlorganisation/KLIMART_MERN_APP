...
function HomePageSec() {
  ...
  return loading ? (
    ...
    >
      ...
      <div class="card">
        <div class="card-body">
          {index==3 ||index==9 ? <p style={{color:"red"}}>No Image is needed for this section</p>: <img
          src={homeData?.Image[0]}
          class="card-img-top"
          alt="..."
          style={{ height: "350px" }}
        />}
        {index==3 ? <p style={{color:"red"}}>No header is needed for this section</p>:<h5 class="card-title">{homeData?.header}</h5>}
          
          {index==8 ? <p style={{color:"red"}} >No description is needed for this section</p>: <p class="card-text">{descData ? homeData?.Desc  : homeData?.Desc?.slice(0,10)  }{"   "}
      ...
      <HomeModal homeData={homeData} onUpdateData = {handleHomeSection} index={index}/>
    </div>
      </>
