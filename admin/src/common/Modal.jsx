...
function Modal({aboutData , onUpdateData}) {
  const [updateData, setUpdateData] = useState(aboutData);
  ...
  useEffect(()=>{
    setUpdateData(aboutData)
    setImagePreview(aboutData?.Image[0])
  ...
  const handleUpdate = async()=>{
    try{
      ...
      console.log("props?.updateData?._id" , aboutData?._id)
  
      const {data} = await instance.put(`/aboutpage/${aboutData?._id}` , formData , {withCredentials:true})   
