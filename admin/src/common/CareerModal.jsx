...
function CareerModal({careerData , onUpdateData}) {
  const [updateData, setUpdateData] = useState(careerData);
  ...
  useEffect(()=>{
    setUpdateData(careerData)
    setImagePreview(careerData?.Image[0])
  ...
  const handleUpdate = async()=>{
    try{
      ...
      console.log("props?.updateData?._id" , careerData?._id)
  
      const {data} = await instance.put(`/careerpage/${careerData?._id}` , formData , {withCredentials:true})   
